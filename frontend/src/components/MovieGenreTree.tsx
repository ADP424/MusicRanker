...
function computeLayout(genres: MovieGenre[]) {
  ...
  const byDepth = new Map<number, MovieGenre[]>();
  for (const g of genres) {
...
function Edge({ from, to, nodeMap, highlighted, dimmed }: {
...
export function MovieGenreTree({ genres, onEdit }: { genres: MovieGenre[]; onEdit: (g: MovieGenre) => void }) {
  ...
  const connectedIds = useMemo(() => {
    ...
    for (const e of edges) {
      if (e.from === selected || e.to === selected) { ids.add(e.from); ids.add(e.to); }
    }
    return ids;
  }, [selected, edges]);
  ...
  function onNodeClick(id: number, e: React.MouseEvent) {
    e.stopPropagation(); if (moved.current) return;
    setSelected((s) => (s === id ? null : id));
  }

  return (
    <div className="genre-tree-canvas"
      onMouseDown={onMouseDown} onMouseMove={onMouseMove}
      onMouseUp={onMouseUp} onMouseLeave={onMouseUp}
      onWheel={onWheel} onClick={() => { if (!moved.current) setSelected(null); }}
    >
      <div style={{ transform: `translate(${offset.x}px, ${offset.y}px) scale(${zoom})`, transformOrigin: "0 0", position: "relative", width, height }}>
        <svg style={{ position: "absolute", inset: 0, overflow: "visible", pointerEvents: "none" }} width={width} height={height}>
          {edges.map((e) => {
            const hl = connectedIds !== null && connectedIds.has(e.from) && connectedIds.has(e.to);
            const dm = connectedIds !== null && !hl;
            return <Edge key={`${e.from}-${e.to}`} from={e.from} to={e.to} nodeMap={nodeMap} highlighted={hl} dimmed={dm} />;
          })}
        </svg>
        {nodes.map((n) => {
          ...
          return (
            <div key={n.id}
              className={"genre-node" + (isSelected ? " genre-node-selected" : "") + (isDimmed ? " genre-node-dimmed" : "")}
            ...
            >
              ...
              <button className="genre-node-edit icon" onClick={(e) => { e.stopPropagation(); onEdit(n.genre); }} title="Edit">✎</button>
            </div>
          );
        })}
      </div>
